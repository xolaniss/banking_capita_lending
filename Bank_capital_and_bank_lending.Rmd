...
```{r}
#| capital,
#| fig.cap = "Capital requirements. Source: South African Reserve Bank (2022)",
#| cache = FALSE,
#| include = TRUE,
#| fig.height = 3
...
Capital_monthly_tbl <- preprocessing_monthly$Totals$capital_buffer_tbl %>% 
  ...
  rename(
    "Minimum capital requirement" = `Minimum requirement`,
    "Surplus capital held" = `Surplus capital`)
...
flextable_word(
  data = summary_tbl_id,
  caption = "Definitions and Summary Statistics") %>%
...
```{r}
#| path,
#| fig.cap = "Local projections (impulse responses)",
#| cache = FALSE,
#| include = TRUE,
#| eval = TRUE,
#| fig.height = 6
...
flextable(
  ...
  household_mortgage_model$body$dataset
    ...
    add_row(.before = 1," " = "Household secured credit model") %>% 
    add_row(.after = 10, " " = "Household unsecured credit model") %>% 
    add_row(.after = 20, " " = "Household mortgage credit model") %>% 
    add_row(.after = 10, 
            " " = "Adj.R squared",  
            "(1)" = as.character(household_secured_r_squared_vec[1]), 
            "(2)" = as.character(household_secured_r_squared_vec[2]),
  ...
  hline(i = 1, border = border) %>% 
  hline(i = 9, border = border) %>% 
  hline(i = 12, border = border) %>% 
  hline(i = 13, border = border) %>% 
  hline(i = 21, border = border)  %>%  
  hline(i = 24, border = border) %>% 
  hline(i = 25, border = border) %>% 
  hline(i = 33, border = border) %>% 
  bold(i = c(1, 13, 25)) %>% 
  # set_header_labels(" " = "",
  #                   "(1)" = "1 Month Lag",
  ...
  mk_par(j = 1, i= c(2, 4, 6, 8), part = "body", value = as_paragraph(as_equation(estimates
                                                                   ))) %>% 
  mk_par(j = 1, i= c(14, 16, 18, 20), part = "body", value = as_paragraph(as_equation(estimates
                                                                   ))) %>% 
  mk_par(j = 1, i= c(26, 28, 30, 32), part = "body", value = as_paragraph(as_equation(estimates
                                                                   ))) %>% 
  flextable::align(part = "body", align = "center") %>% 
  flextable::align(part = "header", align = "center") %>% 
  flextable::align(j =1, part = "body", align = "left") %>% 
...
flextable(
  ...
  corporation_mortgage_model$body$dataset
    ...
    add_row(.before = 1," " = "Non-Financial corporations secured credit model") %>% 
    add_row(.after = 10, " " = "Non-Financial corporations unsecured credit model") %>% 
    add_row(.after = 20, " " = "Non-Financial corporations mortgage credit model") %>% 
    add_row(.after = 10, 
            " " = "Adj.R squared",  
            "(1)" = as.character(corporation_secured_r_squared_vec[1]), 
            "(2)" = as.character(corporation_secured_r_squared_vec[2]),
